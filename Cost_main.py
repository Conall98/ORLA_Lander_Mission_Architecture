...
#%% Results function
def results(MA, n, tvpow,  MA_star = None):
    ...
    if MA_star is not None:
        MA_star_costs = cost_main(MA_star, n, tvpow)
        SPDC = MA_star_costs["Life Cycle Cost [M€]"]/(MA[0].mp*n)*10e6
        SPRC = ((MA_costs["Life Cycle Cost [M€]"] - MA_star_costs["Life Cycle Cost [M€]"])/(MA[0].mp*n))*10e6
    else:    
